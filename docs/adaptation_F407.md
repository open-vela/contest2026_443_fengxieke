...
- **换算**：ACS712-30A（66mV/A，5V 供电）输出经 10kΩ+10kΩ 分压后进 PA4，
  分压后零点约 1.16~1.25V（2026-09-18 实测 1.161V，此时 5V 实际约 4.64V）、灵敏度 33mV/A，上电做一次零点校准；
  温度用 10kΩ + NTC(10K B3950) 分压，按 B 参数方程换算。
- **输出**：PB0 继电器（**高电平触发**，宏 `FIREEYE_RELAY_ACTIVE_LOW = 0`；上电默认吸合、负载断电），
...
| 报警灯模块（PD9） | `fireeye test` + `fireeye level` | 亮灭受控；**模块为高电平触发**（`FIREEYE_LED_ACTIVE_HIGH = 1`，实测 HIGH 点亮）|
| 蜂鸣器（PB1） | `fireeye test` + 实机加热触发 | 短鸣/长鸣/间歇音型均正常；**模块为低电平触发**（板级宏 `FIREEYE_BUZZER_ACTIVE_HIGH = 0`）|
| 继电器联动（PB0） | `fireeye test` + 12V 风扇负载 | 吸合释放正常，风扇可被切断；**模块为高电平触发**（`FIREEYE_RELAY_ACTIVE_LOW = 0`，2026-09-18 定标）|
...
用防水 NTC 泡温水逐级触发，实测与阈值一致：

| 水温 | 系统反应 |
|---|---|
| < 45℃ | NORMAL：风扇转、灯灭、蜂鸣器静音，串口 state=NORMAL |
| 50℃ | WARNING：蜂鸣器间歇报警、报警灯慢闪，风扇继续转（不切断）|
| 60℃ 以上 | ALARM：蜂鸣器长鸣 + 报警灯常亮 + 继电器吸合切断风扇（联动断电）|
| 按板载 K2 | 复位回 NORMAL，报警停止、负载恢复供电 |

输出模块的触发极性在不同批次模块上不一致，调试时用 `fireeye level` 命令逐脚输出
LOW/HIGH 各 2 秒即可确认；本项目实测：继电器高电平触发、蜂鸣器低电平触发、报警灯高电平触发。

另外修正一处演示缺陷：**电流零点必须在负载不工作时校准**。原先在应用启动时直接采样，
若此时负载（风扇）已在运行，会把负载电流当成零点（现象：负载停转后读数变成 -0.16A）。
现改为开机先让继电器吸合 0.5 秒断开负载、采样零点后再恢复供电。
